...
export function SkillsPage() {
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsAdding(false);
      setSavedMessage("Skill saved. Your dashboard has updated.");
      setNewSkillName("");
      setNewSkillCategory("technical");
      setNewSkillLevel("beginner");
    } catch (err) {
      setSaveError(
        "Could not save this skill. Your entries are kept; please retry.",
      );
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <Link to="/records?kind=skill" className="btn btn-secondary">
        Update skill confidence and levels
      </Link>
      ...
      {skills.length === 0 ? (
        ...
        <div className="skills-grid">
          {filteredSkills.map((skill, index) => {
            const levelColor =
              SKILL_LEVEL_COLORS[skill.level as SkillLevel] ||
              SKILL_LEVEL_COLORS.beginner;
