...
interface ActorContextType {
  ...
  addEvent: (
    event: Omit<ActorEvent, "id" | "createdAt" | "userId">,
  ) => Promise<void>;
...
export function ActorProvider({ children }: { children: ReactNode }) {
  ...
  const actorState =
    rawActorState || (user ? { ...DEFAULT_STATE, userId: user.id } : null);

  useEffect(() => {
    if (!user || rawActorState !== null) return;
    void db
      .transaction("rw", db.actor_states, async () => {
        if (!(await db.actor_states.where("userId").equals(user.id).first())) {
      ...
      .catch((error) =>
        console.error("Could not initialize ACTOR state", error),
      );
  }, [user?.id, rawActorState]);
